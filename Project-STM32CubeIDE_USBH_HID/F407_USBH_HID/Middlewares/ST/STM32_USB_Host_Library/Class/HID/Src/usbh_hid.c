...
//		uint8_t num_endpoints = interface_x->bNumEndpoints;
//		uint8_t ep_index;
//		for (ep_index = 0; ep_index < num_endpoints; ep_index++) {
//			USBH_EpDescTypeDef *ep_desc = &interface_x->Ep_Desc[ep_index];
//			USBH_UsrLog(
//					"Interface %u, endpoint #%u: address 0x%02x, attributes 0x%02x\n",
//					interface_index, ep_index, ep_desc->bEndpointAddress,
...
//interface = interface_index;
		...
		if (phost->device.CfgDesc.Itf_Desc[interface_index].bInterfaceProtocol
				== HID_KEYBRD_BOOT_CODE) {
			USBH_UsrLog("KeyBoard device found!");
			HID_Handle->Init = USBH_HID_KeybdInit;
		} else if (phost->device.CfgDesc.Itf_Desc[interface_index].bInterfaceProtocol
				== HID_MOUSE_BOOT_CODE) {
