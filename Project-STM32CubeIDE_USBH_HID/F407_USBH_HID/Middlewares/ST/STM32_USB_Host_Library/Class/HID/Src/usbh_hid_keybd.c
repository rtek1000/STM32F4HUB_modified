...
static const uint8_t HID_KEYBRD_Key[] = { '\0', '`', '1', '2', '3', '4', '5',
...
static const uint8_t HID_KEYBRD_ShiftKey[] = { '\0', '~', '!', '@', '#', '$',
		'%', '^', '&', '*', '(', ')', '_', '+', '\0', '\0', '\0', 'Q', 'W', 'E',
...
USBH_StatusTypeDef USBH_HID_KeybdInit(USBH_HandleTypeDef *phost) {
	...
	if (HID_Handle->length_array[0] > (sizeof(keybd_report_data))) {
		HID_Handle->length_array[0] = (uint16_t) (sizeof(keybd_report_data));
	}
...
static USBH_StatusTypeDef USBH_HID_KeybdDecode(USBH_HandleTypeDef *phost) {
	...
	/*Fill report */
	if (USBH_HID_FifoRead(&HID_Handle->fifo, &keybd_report_data,
			HID_Handle->length_array[0]) == HID_Handle->length_array[0]) {
		keybd_info.lctrl = (uint8_t) HID_ReadItem(
				(HID_Report_ItemTypedef*) &imp_0_lctrl, 0U);
		keybd_info.lshift = (uint8_t) HID_ReadItem(
				(HID_Report_ItemTypedef*) &imp_0_lshift, 0U);
		keybd_info.lalt = (uint8_t) HID_ReadItem(
				(HID_Report_ItemTypedef*) &imp_0_lalt, 0U);
		keybd_info.lgui = (uint8_t) HID_ReadItem(
				(HID_Report_ItemTypedef*) &imp_0_lgui, 0U);
		keybd_info.rctrl = (uint8_t) HID_ReadItem(
				(HID_Report_ItemTypedef*) &imp_0_rctrl, 0U);
		keybd_info.rshift = (uint8_t) HID_ReadItem(
				(HID_Report_ItemTypedef*) &imp_0_rshift, 0U);
		keybd_info.ralt = (uint8_t) HID_ReadItem(
				(HID_Report_ItemTypedef*) &imp_0_ralt, 0U);
