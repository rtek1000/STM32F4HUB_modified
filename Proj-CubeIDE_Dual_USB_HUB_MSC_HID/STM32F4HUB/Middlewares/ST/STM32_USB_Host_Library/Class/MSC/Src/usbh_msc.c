...
extern uint8_t USBHostMscId;
extern uint8_t USBDISKFatReady;
/** @addtogroup USBH_LIB
 * @{
 */
...
USB_MSC_CLASS, USBH_MSC_InterfaceInit, USBH_MSC_InterfaceDeInit,
		USBH_MSC_ClassRequest, USBH_MSC_Process, USBH_MSC_SOFProcess,
		NULL, };
...
static USBH_StatusTypeDef USBH_MSC_InterfaceInit(USBH_HandleTypeDef *phost) {
	...
	phost->pActiveClass->pData = (MSC_HandleTypeDef*) USBH_malloc(
		...
		if (status == USBH_OK) {
			MSC_Handle->max_lun =
					(MSC_Handle->max_lun > MAX_SUPPORTED_LUN) ?
							MAX_SUPPORTED_LUN : (MSC_Handle->max_lun + 1U);
	...
	case MSC_INIT:
		...
		if (MSC_Handle->current_lun < MSC_Handle->max_lun) {
			...
			case MSC_READ_INQUIRY:
				scsi_status = USBH_MSC_SCSI_Inquiry(phost,
						(uint8_t) MSC_Handle->current_lun,
				...
				if (scsi_status == USBH_FAIL) {
					...
					if (scsi_status == USBH_UNRECOVERED_ERROR) {
						MSC_Handle->unit[MSC_Handle->current_lun].state =
								MSC_IDLE;
						MSC_Handle->unit[MSC_Handle->current_lun].error =
								MSC_ERROR;
					}
				}
				break;
			...
			case MSC_TEST_UNIT_READY:
				ready_status = USBH_MSC_SCSI_TestUnitReady(phost,
						(uint8_t) MSC_Handle->current_lun);

				if (ready_status == USBH_OK) {
					if (MSC_Handle->unit[MSC_Handle->current_lun].prev_ready_state
							!= USBH_OK) {
						MSC_Handle->unit[MSC_Handle->current_lun].state_changed =
								1U;
						USBH_UsrLog("MSC Device ready");
						USBHostMscId = phost->id;
						USBDISKFatReady = USBH_OK;
					} else {
						MSC_Handle->unit[MSC_Handle->current_lun].state_changed =
								0U;
					}
			...
			case MSC_READ_CAPACITY10:
				scsi_status = USBH_MSC_SCSI_ReadCapacity(phost,
						(uint8_t) MSC_Handle->current_lun,
						&MSC_Handle->unit[MSC_Handle->current_lun].capacity);

				if (scsi_status == USBH_OK) {
					if (MSC_Handle->unit[MSC_Handle->current_lun].state_changed
							== 1U) {
			...
			case MSC_REQUEST_SENSE:
				scsi_status = USBH_MSC_SCSI_RequestSense(phost,
						(uint8_t) MSC_Handle->current_lun,
...
uint8_t USBH_MSC_UnitIsReady(USBH_HandleTypeDef *phost, uint8_t lun) {
	...
	if ((phost->gState == HOST_CLASS)
			&& (MSC_Handle->unit[lun].error == MSC_OK)) {
		res = 1U;
	} else {
		res = 0U;
	}
...
USBH_StatusTypeDef USBH_MSC_GetLUNInfo(USBH_HandleTypeDef *phost, uint8_t lun,
	...
	MSC_HandleTypeDef *MSC_Handle =
			(MSC_HandleTypeDef*) phost->pActiveClass->pData;
	if (phost->gState == HOST_CLASS) {
		(void) USBH_memcpy(info, &MSC_Handle->unit[lun],
				sizeof(MSC_LUNTypeDef));
		return USBH_OK;
	} else {
		return USBH_FAIL;
	}
...
USBH_StatusTypeDef USBH_MSC_Read(USBH_HandleTypeDef *phost, uint8_t lun,
	...
	while (USBH_MSC_RdWrProcess(phost, lun) == USBH_BUSY) {
		if (((phost->Timer - timeout) > (10000U * length))
