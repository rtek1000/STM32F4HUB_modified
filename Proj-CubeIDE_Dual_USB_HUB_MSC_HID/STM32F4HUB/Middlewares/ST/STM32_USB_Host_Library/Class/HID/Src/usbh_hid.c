...
static USBH_StatusTypeDef USBH_HID_Parameter(USBH_HandleTypeDef *phost,
		uint8_t param, uint8_t *data, uint16_t *length);
...
static USBH_StatusTypeDef create_endpoints(USBH_HandleTypeDef *phost,
		HID_HandleTypeDef *HID_Handle, USBH_InterfaceDescTypeDef *pif) {
	uint8_t max_ep;
	uint8_t num = 0;
	...
	max_ep = (
			(pif->bNumEndpoints <= USBH_MAX_NUM_ENDPOINTS) ?
					pif->bNumEndpoints : USBH_MAX_NUM_ENDPOINTS);
	...
	for (num = 0; num < max_ep; num++) {
		if (pif->Ep_Desc[num].bEndpointAddress & 0x80) {
			...
			USBH_OpenPipe(phost, HID_Handle->InPipe, HID_Handle->InEp,
					...
					USB_EP_TYPE_INTR, HID_Handle->length[0]);

			USBH_LL_SetToggle(phost, HID_Handle->InPipe, 0);

			USBH_UsrLog(
					"IN  size=%d, num %d, addr 0x%02X, pool %d, lowspeed? %d",

					HID_Handle->length[0], HID_Handle->InPipe,
					HID_Handle->ep_addr[0], HID_Handle->poll[0],
					(phost->device.speed == 2));
			...
			USBH_OpenPipe(phost, HID_Handle->OutPipe, HID_Handle->OutEp,
					phost->device.address, phost->device.speed,
					USB_EP_TYPE_INTR, HID_Handle->length[1]);

			USBH_LL_SetToggle(phost, HID_Handle->OutPipe, 0);

			USBH_UsrLog("OUT size=%d, num %d, addr 0x%02X, pool %d",
					HID_Handle->length[1], HID_Handle->OutPipe,
					HID_Handle->ep_addr[1], HID_Handle->poll[1]);
		}
	}
...
static HID_HandleTypeDef* create_handle(USBH_HandleTypeDef *phost) {
	phost->USBH_ClassTypeDef_pData[phost->interfaces] =
			(HID_HandleTypeDef*) USBH_malloc(sizeof(HID_HandleTypeDef));
	memset(phost->USBH_ClassTypeDef_pData[phost->interfaces], 0,
			sizeof(HID_HandleTypeDef));
	HID_HandleTypeDef *p =
			(HID_HandleTypeDef*) phost->USBH_ClassTypeDef_pData[phost->interfaces];
	p->state = HID_ERROR;
	phost->interfaces++;
	return p;
}
...
static USBH_StatusTypeDef USBH_HID_InterfaceInit(USBH_HandleTypeDef *phost) {
	...
	case 1:
		...
		USBH_UsrLog("Class    : %xh",
				phost->device.CfgDesc.Itf_Desc[if_ix].bInterfaceClass);
		USBH_UsrLog("SubClass : %xh",
				phost->device.CfgDesc.Itf_Desc[if_ix].bInterfaceSubClass);
		USBH_UsrLog("Protocol : %xh",
				phost->device.CfgDesc.Itf_Desc[if_ix].bInterfaceProtocol);

		break;

	case 2:
		/* Get HID Desc */
		if (USBH_HID_GetHIDDescriptor(phost, itfnum, USB_HID_DESC_SIZE)
				== USBH_OK) {
			USBH_HID_ParseHIDDesc(&hid_desc, phost->device.Data);
			USBH_UsrLog("HID LEN %d - %d", hid_desc.wItemLength,
	...
	case 3:
		/* Get Report Desc */
		if (USBH_HID_GetHIDReportDescriptor(phost, itfnum, hid_desc.wItemLength)
				== USBH_OK) {
			pif = &pcfg->Itf_Desc[if_ix];

			if (pif->bInterfaceClass
					== 0x03&& pif->bInterfaceSubClass == 0x01 && pif->bInterfaceProtocol == HID_KEYBRD_BOOT_CODE) {
				USBH_UsrLog("KeyBoard found.");
				HID_Handle = create_handle(phost);
				HID_Handle->Init = USBH_HID_KeybdInit;
			} else if (pif->bInterfaceClass
					== 0x03&& pif->bInterfaceSubClass == 0x01 && pif->bInterfaceProtocol == HID_MOUSE_BOOT_CODE) {
...
//Usage (Mouse) 09 02
				uint16_t usage_page = phost->device.Data[0] << 8
						| phost->device.Data[1];
				uint16_t usage = phost->device.Data[2] << 8
						| phost->device.Data[3];
	...
	case 5:
		...
		USBH_SelectInterface(phost,
				phost->device.CfgDesc.Itf_Desc[0].bInterfaceNumber);
		state = 0;
		status = USBH_OK;
		break;
	}
...
USBH_StatusTypeDef USBH_HID_GetHIDReportDescriptor(USBH_HandleTypeDef *phost,
	...
	USB_DESC_HID_REPORT, interface, phost->device.Data, length);
...
USBH_StatusTypeDef USBH_HID_GetHIDDescriptor(USBH_HandleTypeDef *phost,
	...
	status = USBH_GetDescriptor(phost,
	USB_REQ_RECIPIENT_INTERFACE | USB_REQ_TYPE_STANDARD,
	USB_DESC_HID, interface, phost->device.Data, length);
	return status;
}

static USBH_StatusTypeDef hid_request(USBH_HandleTypeDef *phost,
		uint8_t request, uint8_t dataDirection, uint16_t value, uint16_t index,
		uint8_t *buffer, uint16_t size) {
	phost->Control.setup.b.bmRequestType = dataDirection
			| USB_REQ_RECIPIENT_INTERFACE | USB_REQ_TYPE_CLASS;
...
USBH_StatusTypeDef USBH_HID_SetIdle(USBH_HandleTypeDef *phost, uint8_t duration,
		uint8_t reportId) {
	return hid_request(phost, USB_HID_SET_IDLE, USB_H2D,
			(duration << 8) | reportId, 0, 0, 0);
}
...
USBH_StatusTypeDef USBH_HID_SetReport(USBH_HandleTypeDef *phost,
		...
		uint16_t reportLen) {
	return hid_request(phost, USB_HID_SET_REPORT, USB_H2D,
			(reportType << 8) | reportId, 0, reportBuff, reportLen);
}
...
USBH_StatusTypeDef USBH_HID_GetReport(USBH_HandleTypeDef *phost,
		uint8_t reportType, uint8_t reportId, uint8_t *reportBuff,
		uint16_t reportLen) {
	return hid_request(phost, USB_HID_GET_REPORT, USB_D2H,
			(reportType << 8) | reportId, 0, reportBuff, reportLen);
}
...
USBH_StatusTypeDef USBH_HID_SetProtocol(USBH_HandleTypeDef *phost,
		uint8_t protocol) {
	return hid_request(phost, USB_HID_SET_PROTOCOL, USB_H2D,
			protocol != 0 ? 0 : 1, 0, 0, 0);
}
...
USBH_StatusTypeDef USBH_HID_Parameter(USBH_HandleTypeDef *phost, uint8_t param,
