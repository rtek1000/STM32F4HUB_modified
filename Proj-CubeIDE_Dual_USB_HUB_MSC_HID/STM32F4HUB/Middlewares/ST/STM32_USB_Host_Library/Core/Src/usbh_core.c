...
USBH_StatusTypeDef USBH_Process(USBH_HandleTypeDef *phost) {
	...
	case HOST_DEV_WAIT_FOR_ATTACHMENT:
		if ((HAL_GetTick() - timeout) > 2000) {
			...
			USBH_DbgLog("HOST_DEV_WAIT_FOR_ATTACHMENT timeout");
	...
	case HOST_DEV_ATTACHED:
		...
		/* Open Control pipes */
		USBH_OpenPipe(phost, phost->Control.pipe_out, 0x00,
				phost->device.address, phost->device.speed,
				USBH_EP_CONTROL, phost->Control.pipe_size);
	...
	case HOST_ENUMERATION:
		if ((HAL_GetTick() - timeout) > 2000) {
			...
			USBH_DbgLog("HOST_ENUMERATION timeout");
			...
			for (idx = 0; idx < USBH_MAX_NUM_SUPPORTED_CLASS; idx++) {
				if (phost->pClass[idx] != NULL) {
					if ((phost->pClass[idx]->ClassCode
							...
							|| (phost->pClass[idx]->ClassCode == 0x03
				...
				if (phost->id == ID_USB_HOST_FS) {
					if (hUSBHostFS[i].valid) {
						if (hUSBHostFS[i].pActiveClass != NULL) {
							hUSBHostFS[i].pActiveClass->DeInit(&hUSBHostFS[i]);
							hUSBHostFS[i].pActiveClass = NULL;
						}

						memset(&hUSBHostFS[i], 0, sizeof(USBH_HandleTypeDef));
					}
				}
				if (phost->id == ID_USB_HOST_HS) {
					if (hUSBHostHS[i].valid) {
						if (hUSBHostHS[i].pActiveClass != NULL) {
							hUSBHostHS[i].pActiveClass->DeInit(&hUSBHostHS[i]);
							hUSBHostHS[i].pActiveClass = NULL;
						}
...
static USBH_StatusTypeDef USBH_HandleEnum(USBH_HandleTypeDef *phost) {
	...
	case ENUM_IDLE:
		...
		if (USBH_Get_DevDesc(phost, 8) == USBH_OK) {
			...
			USBH_UsrLog("Enum mps: %d, addr: %d", phost->Control.pipe_size,
...
//USBH_UsrLog("CLASS: %xh", phost->device.DevDesc.bDeviceClass );
	...
	case ENUM_GET_MFC_STRING_DESC:
		if (phost->device.DevDesc.iManufacturer != 0) { /* Check that Manufacturer String is available */
	...
	case ENUM_GET_PRODUCT_STRING_DESC:
		if (phost->device.DevDesc.iProduct != 0) { /* Check that Product string is available */
	...
	case ENUM_GET_SERIALNUM_STRING_DESC:
		if (phost->device.DevDesc.iSerialNumber != 0) { /* Check that Serial number string is available */
