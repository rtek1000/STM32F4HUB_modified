...
USB_CDC_CLASS, USBH_CDC_InterfaceInit, USBH_CDC_InterfaceDeInit,
		USBH_CDC_ClassRequest, USBH_CDC_Process, USBH_CDC_SOFProcess,
		NULL, };
/**
 * @}
 */
...
static USBH_StatusTypeDef USBH_CDC_InterfaceInit(USBH_HandleTypeDef *phost) {
	...
	{
		USBH_DbgLog(
				"Cannot Find the interface for Communication Interface Class: %s",
	...
	(void) USBH_OpenPipe(phost, CDC_Handle->CommItf.NotifPipe,
	...
	if ((interface == 0xFFU) || (interface >= USBH_MAX_NUM_INTERFACES)) /* No Valid Interface */
	{
		USBH_DbgLog("Cannot Find the interface for Data Interface Class: %s",
				phost->pActiveClass->Name);
		return USBH_FAIL;
	}
	...
	(void) USBH_OpenPipe(phost, CDC_Handle->DataItf.OutPipe,
			CDC_Handle->DataItf.OutEp, phost->device.address,
			phost->device.speed, USB_EP_TYPE_BULK,
			CDC_Handle->DataItf.OutEpSize);

	/* Open channel for IN endpoint */
	(void) USBH_OpenPipe(phost, CDC_Handle->DataItf.InPipe,
...
static USBH_StatusTypeDef USBH_CDC_ClassRequest(USBH_HandleTypeDef *phost) {
	...
	if (status == USBH_OK) {
		...
		USBH_ErrLog(
				"Control error: CDC: Device Get Line Coding configuration failed");
	} else {
		/* .. */
	}
...
static void CDC_ProcessTransmission(USBH_HandleTypeDef *phost) {
	...
	case CDC_SEND_DATA:
		if (CDC_Handle->TxDataLength > CDC_Handle->DataItf.OutEpSize) {
			(void) USBH_BulkSendData(phost, CDC_Handle->pTxData,
					CDC_Handle->DataItf.OutEpSize, CDC_Handle->DataItf.OutPipe,
					1U);
		} else {
			(void) USBH_BulkSendData(phost, CDC_Handle->pTxData,
					(uint16_t) CDC_Handle->TxDataLength,
					CDC_Handle->DataItf.OutPipe, 1U);
		}
...
static void CDC_ProcessReception(USBH_HandleTypeDef *phost) {
	...
	case CDC_RECEIVE_DATA:
		...
		(void) USBH_BulkReceiveData(phost, CDC_Handle->pRxData,
	...
	case CDC_RECEIVE_DATA_WAIT:
		...
		if (URB_Status == USBH_URB_DONE) {
			...
			if (((CDC_Handle->RxDataLength - length) > 0U)
