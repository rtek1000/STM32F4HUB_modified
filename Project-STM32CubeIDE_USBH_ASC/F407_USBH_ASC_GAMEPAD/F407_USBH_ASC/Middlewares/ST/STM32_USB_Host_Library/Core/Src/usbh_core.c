...
extern void MX_USB_HOST_Init_FS(void);
...
uint8_t USBH_CheckInterface(USBH_HandleTypeDef *phost, uint8_t interface_number,
		uint8_t Class, uint8_t SubClass, uint8_t Protocol) {
	USBH_InterfaceDescTypeDef *pif;
	USBH_CfgDescTypeDef *pcfg;
	uint8_t if_ix = interface_number;
...
USBH_StatusTypeDef USBH_Process(USBH_HandleTypeDef *phost) {
	...
	case HOST_DEV_WAIT_FOR_ATTACHMENT: /* Wait for Port Enabled */
		if (phost->device.PortEnabled == 1U) {
			...
			if ((USBH_GetTick() - phost->Timeout) > USBH_DEV_RESET_TIMEOUT) {
				phost->device.RstCnt++;
	...
	case HOST_DEV_ATTACHED:
		...
		if (phost->resetPortPhase == 0U) {
			...
			if ((USBH_GetTick() - phost->resetPortTimer) > 100U) {
				phost->resetPortPhase = 2U;
			}
		} else if (phost->resetPortPhase == 2U) {
			phost->resetPortPhase = 0U;
	...
	case HOST_ENUMERATION:
        ...
        (void)osMessageQueuePut(phost->os_event, &phost->os_msg, 0U, 0U);
			...
			if ((USBH_GetTick() - phost->Timeout) > USBH_DEV_RESET_TIMEOUT) {
				//USBH_UsrLog("USB Enumeration Failed, Please unplug the Device.");
				phost->gState = HOST_ABORT_STATE;
				phost->Timeout = USBH_GetTick(); //0;
				//} else {
				//phost->Timeout += 10U;
				//USBH_Delay(10U);
			}
