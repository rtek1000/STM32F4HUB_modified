  ...
  switch (MTP_Handle->state)
  {
    ...
    case MTP_GETDEVICEINFO:
      ...
      if (status == USBH_OK)
      {
        ...
        USBH_DbgLog("Number of Supported Operation(s) : %lu", MTP_Handle->info.devinfo.OperationsSupported_len);
        USBH_DbgLog("Number of Supported Events(s) : %lu", MTP_Handle->info.devinfo.EventsSupported_len);
        USBH_DbgLog("Number of Supported Proprieties : %lu", MTP_Handle->info.devinfo.DevicePropertiesSupported_len);
    ...
    case MTP_GETSTORAGEIDS:
      ...
      if (status == USBH_OK)
      {
        USBH_DbgLog("Number of storage ID items : %lu", MTP_Handle->info.storids.n);
        for (idx  = 0U; idx < MTP_Handle->info.storids.n; idx ++)
        {
          USBH_DbgLog("storage#%lu ID : %lx", idx, MTP_Handle->info.storids.Storage[idx]);
        }
