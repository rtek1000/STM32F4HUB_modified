...
HAL_StatusTypeDef HAL_HCD_ResetPort_Start(HCD_HandleTypeDef *hhcd) {
	return (USB_ResetPort_Start(hhcd->Instance));
}

HAL_StatusTypeDef HAL_HCD_ResetPort_End(HCD_HandleTypeDef *hhcd) {
	return (USB_ResetPort_End(hhcd->Instance));
}
...
static void HCD_HC_IN_IRQHandler(HCD_HandleTypeDef *hhcd, uint8_t chnum) {
	...
	if ((USBx_HC(ch_num)->HCINT & USB_OTG_HCINT_AHBERR) == USB_OTG_HCINT_AHBERR) {
		...
		(void) USB_HC_Halt(hhcd->Instance, (uint8_t) ch_num);
	} else if ((USBx_HC(ch_num)->HCINT & USB_OTG_HCINT_BBERR)
			== USB_OTG_HCINT_BBERR) {
	...
	} else if ((USBx_HC(ch_num)->HCINT & USB_OTG_HCINT_STALL)
			== USB_OTG_HCINT_STALL) {
		__HAL_HCD_CLEAR_HC_INT(ch_num, USB_OTG_HCINT_STALL);
		hhcd->hc[ch_num].state = HC_STALL;
		(void) USB_HC_Halt(hhcd->Instance, (uint8_t) ch_num);
	} else if ((USBx_HC(ch_num)->HCINT & USB_OTG_HCINT_DTERR)
			== USB_OTG_HCINT_DTERR) {
		__HAL_HCD_CLEAR_HC_INT(ch_num, USB_OTG_HCINT_DTERR);
		hhcd->hc[ch_num].state = HC_DATATGLERR;
		(void) USB_HC_Halt(hhcd->Instance, (uint8_t) ch_num);
	} else if ((USBx_HC(ch_num)->HCINT & USB_OTG_HCINT_TXERR)
			== USB_OTG_HCINT_TXERR) {
	...
	if ((USBx_HC(ch_num)->HCINT & USB_OTG_HCINT_FRMOR) == USB_OTG_HCINT_FRMOR) {
      ...
      hhcd->HC_NotifyURBChangeCallback(hhcd, (uint8_t)ch_num, hhcd->hc[ch_num].urb_state);
		...
		if (hhcd->Init.dma_enable == 1U) {
			if (((hhcd->hc[ch_num].XferSize / hhcd->hc[ch_num].max_packet) & 1U)
					!= 0U) {
...
static void HCD_HC_OUT_IRQHandler(HCD_HandleTypeDef *hhcd, uint8_t chnum) {
	...
	if ((USBx_HC(ch_num)->HCINT & USB_OTG_HCINT_AHBERR) == USB_OTG_HCINT_AHBERR) {
		...
		hhcd->hc[ch_num].state = HC_XFRC;
		(void) USB_HC_Halt(hhcd->Instance, (uint8_t) ch_num);
	} else if ((USBx_HC(ch_num)->HCINT & USB_OTG_HCINT_NYET)
			== USB_OTG_HCINT_NYET) {
		...
		if (hhcd->hc[ch_num].state == HC_XFRC) {
			...
			if ((hhcd->hc[ch_num].ep_type == EP_TYPE_BULK)
				...
				if ((hhcd->Init.dma_enable == 1U)
						&& (hhcd->hc[ch_num].xfer_len > 0U)) {
					num_packets = (hhcd->hc[ch_num].xfer_len
							+ hhcd->hc[ch_num].max_packet - 1U)
...
static void HCD_RXQLVL_IRQHandler(HCD_HandleTypeDef *hhcd) {
	...
	case GRXSTS_PKTSTS_IN:
		...
		if ((pktcnt > 0U) && (hhcd->hc[ch_num].xfer_buff != (void*) 0)) {
			if ((hhcd->hc[ch_num].xfer_count + pktcnt)
					<= hhcd->hc[ch_num].xfer_len) {
				...
				xferSizePktCnt = (USBx_HC(ch_num)->HCTSIZ
						& USB_OTG_HCTSIZ_PKTCNT) >> 19;

				if ((hhcd->hc[ch_num].max_packet == pktcnt)
						&& (xferSizePktCnt > 0U)) {
